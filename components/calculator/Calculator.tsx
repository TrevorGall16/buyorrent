...
export default function Calculator({
  ...
  const defaultInputs = useMemo(
    () => getDefaultInputsForCountry(countryCode, defaultHomePrice, defaultMonthlyRent),
    [countryCode, defaultHomePrice, defaultMonthlyRent]
  );
  ...
  // Hydrate state from URL on EXTERNAL navigation only (Back/Forward).
  // No state variables are read here — only setters are called.
  // React bails out of re-renders when setState receives the same primitive value,
  // so unconditional sets are safe and eliminate the need for comparison logic
  // that previously pulled all state vars into the dependency array (causing the freeze).
  useEffect(() => {
    if (!isMountedRef.current) return;

    // If WE triggered this URL change, skip re-hydration
    if (isInternalUpdateRef.current) {
      isInternalUpdateRef.current = false;
      return;
    }

    // External navigation — push URL values into state
