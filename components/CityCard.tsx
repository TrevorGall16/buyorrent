...
export default function CityCard({ city, countryColor, language = 'en' }: CityCardProps) {
  ...
  return (
    <Link href={`/${city.slug}/buy-vs-rent${language !== 'en' ? `?lang=${language}` : ''}`}>
      <div className="group flex flex-col sm:flex-row overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-xl dark:hover:shadow-2xl dark:hover:shadow-blue-900/20 transition-all duration-300 hover:-translate-y-1 cursor-pointer">
        {/* Left: Image (35%) */}
        ...
        <div className="flex flex-col justify-center p-6 sm:p-8 flex-1">
          ...
          <button className="w-fit px-6 py-3 rounded-full bg-gradient-to-r from-blue-600 to-blue-500 dark:from-blue-500 dark:to-blue-600 text-white font-semibold shadow-md hover:shadow-lg hover:from-blue-700 hover:to-blue-600 dark:hover:from-blue-600 dark:hover:to-blue-700 transition-all duration-200">
            {btnText}
          </button>
        </div>
      </div>
